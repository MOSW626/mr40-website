...
행사정보의 주요 키는 `place`, `address`, `fee`, `refund_policy`, `attendance_count`, `card_payment`, `transfer_payment`, `kakao_map`, `naver_map`, `live_url`, `replay_url`, `program`, `parking`, `seating`, `emergency`입니다. 값이 없으면 화면에는 비활성화된 준비 중 상태가 표시됩니다.
